...
const BEP_FEE = 650;
const BUS_FREIGHT_EXTRA = 1500;
const BUS_COMPLEX_EXTRA = 300;
...
function calculate() {
  ...
  const isBusEnabled = elements.pickupBus.checked;

  const auctionFee = getAuctionFee(auctionType, carPrice);
  const freight = getFreight(auctionType, siteName) + (isBusEnabled ? BUS_FREIGHT_EXTRA : 0);

  const swift = (carPrice + auctionFee) * 0.025 + 100;
  const insurance = carPrice * 0.01 + 150;
  const serviceFee = SERVICE_FEE;
  const bePFee = BEP_FEE;
  const complex = (COMPLEX_BY_CITY[destination] || 0) + (isBusEnabled ? BUS_COMPLEX_EXTRA : 0);

  const isElectric = engineType === 'electric';
  const dangerousCargo = engineType === 'electric' || engineType === 'hybrid' ? 100 : 0;
...
function syncElectricFields() {
  ...
  hazardousRow.classList.add('hidden');
